...
@section('description', 'Oblicz raty swojego kredytu szybko i wygodnie')
...
<section class="hero-section d-flex justify-content-center align-items-center" id="section_1">
            ...
            @auth
                <div style="background-color: rgba(0,0,0,0.7); backdrop-filter: blur(5px); border-radius: 10px; padding: 10px 15px; color: white; display: flex; justify-content: space-between; align-items: center; gap: 15px;">
                    <div style="font-size: 12px;">
                        ...
                        @else
                            <span style="color: #87CEEB;">Uzytkownik - limit: {{ number_format($limit, 0, ',', ' ') }} zł</span>
                        @endif
                    </div>
                    <form method="POST" action="{{ route('logout') }}" style="display: inline;">
                        @csrf
                        <button type="submit" style="background-color: #dc3545; border: none; border-radius: 5px; padding: 5px 10px; color: white; cursor: pointer; font-size: 12px;">Wyloguj</button>
                    </form>
                </div>
            @else
                <div style="background-color: rgba(0,0,0,0.7); backdrop-filter: blur(5px); border-radius: 10px; padding: 10px 15px; color: white; text-align: center;">
                    ...
                    <a href="{{ route('login') }}" style="color: #FFD700; text-decoration: none;">Zaloguj sie</a><br>
                    <span style="font-size: 11px;">Nie masz konta?</span>
                    <a href="{{ route('register') }}" style="color: #FFD700; text-decoration: none;">Zarejestruj sie</a>
                </div>
            @endauth
        </div>
    ...
    <div class="custom-block d-lg-flex flex-column justify-content-center align-items-center">
        <img src="{{ asset('img/wykres.jpg') }}" class="custom-block-image img-fluid" alt="">
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-6 col-12 custom-block-bg-overlay-wrap me-lg-5 mb-5 mb-lg-0">
                <img src="{{ asset('img/zdjecie1.jpg') }}" class="custom-block-bg-overlay-image img-fluid" alt="">
            </div>
            <div class="col-lg-5 col-12 custom-block-bg-overlay-wrap mt-4 mt-lg-0 mb-5 mb-lg-0">
                <img src="{{ asset('img/zdjecie2.jpg') }}" class="custom-block-bg-overlay-image img-fluid" alt="">
            </div>
        </div>
    </div>
</section>
        ...
        <div class="row">
            ...
            <div class="col-lg-12 col-12">
                <div class="services-thumb p-4">
                    <form action="{{ route('kalkulator.oblicz') }}" method="post" class="custom-form">
                        @csrf
                        <div class="row">
                            <div class="col-lg-4 col-12">
                                <label for="kwota" class="form-label">Kwota kredytu (zł):</label>
                                <input type="text" name="kwota" id="kwota" class="form-control" value="{{ old('kwota', $kwota ?? '') }}" placeholder="np. 100000">
                            </div>
                            <div class="col-lg-4 col-12">
                                <label for="oprocentowanie" class="form-label">Oprocentowanie (% rocznie):</label>
                                <input type="text" name="oprocentowanie" id="oprocentowanie" class="form-control" value="{{ old('oprocentowanie', $oprocentowanie ?? '') }}" placeholder="np. 5">
                            </div>
                            <div class="col-lg-4 col-12">
                                <label for="lata" class="form-label">Okres (lata):</label>
                                <input type="text" name="lata" id="lata" class="form-control" value="{{ old('lata', $lata ?? '') }}" placeholder="np. 10">
                            </div>
                            <div class="col-12 mt-4 text-center">
                                @if($user)
                                    <button type="submit" class="btn custom-btn">Oblicz rate</button>
                                @else
                                    <a href="{{ route('login') }}" class="btn custom-btn">Zaloguj się aby obliczyć</a>
                                @endif
                            </div>
                        </div>
                    </form>

                    {{-- wyswietlenie listy bledow --}}
                            ...
                            <div class="row">
                                <div class="col-lg-4 col-12 mb-3">
                                    <div class="services-thumb-price p-3 text-center">
                                        <h5>Odsetki</h5>
                                        <strong class="text-white" style="font-size: 24px;">{{ $wynik['odsetki'] }} zł</strong>
                                    </div>
                                </div>
                                <div class="col-lg-4 col-12 mb-3">
                                    <div class="services-thumb-price p-3 text-center">
                                        <h5>Kwota do spłaty</h5>
                                        <strong class="text-white" style="font-size: 24px;">{{ $wynik['calosc'] }} zł</strong>
                                    </div>
                                </div>
                                <div class="col-lg-4 col-12 mb-3">
                                    <div class="services-thumb-price p-3 text-center">
                                        <h5>Rata miesięczna</h5>
                                        <strong class="text-white" style="font-size: 24px;">{{ $wynik['rata'] }} zł</strong>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-4 col-12 custom-block-bg-overlay-wrap mt-5 mb-5 mb-lg-0 mt-lg-0 pt-3 pt-lg-0">
                <img src="{{ asset('img/lupa.jpg') }}" class="custom-block-bg-overlay-image img-fluid" alt="">
            </div>
        </div>
    </div>
</section>
